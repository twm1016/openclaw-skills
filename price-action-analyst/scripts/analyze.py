...
class InstitutionalAnalystV7_5:
    """
    V7.5 Institutional Pro Ultimate
    - Framework: 15M, 1H, 1D
    - Matrix: Strict 10 Levels (R5-S5)
    - Visualization: Dynamic FVG/OB Rectangles + EMA 12/24
    - Indicators: Vegas Tunnel (144/169, 576/676)
    """
    def __init__(self):
        self.tv = TvDatafeed()
    ...
    def detect_fvg_ob(self, df):
        ...
        for i in range(5, len(df)-1):
            ...
            if body > (df['high'].iloc[i] - df['low'].iloc[i]) * 0.6:
    ...
    def create_chart(self, df, symbol, timeframe, patterns):
        ...
        fig, axlist = mpf.plot(d, type='candle', style='charles', addplot=ap, returnfig=True, figsize=(10, 6), title=f"{symbol} {timeframe} (V7.5 Ultimate)")
        ax = axlist[0]
    ...
    def generate_report(self, symbol):
        ...
        p_15m = self.detect_fvg_ob(df_15m); p_1h = self.detect_fvg_ob(df_1h)
        img15 = self.create_chart(df_15m, asset, "15M", p_15m)
        img1h = self.create_chart(df_1h, asset, "1H", p_1h); img1d = self.create_chart(df_1d, asset, "1D", {'fvg':[], 'ob':[]})
        price = df_15m['close'].iloc[-1]; v_med = df_1h['ema144'].iloc[-1]; v_long = df_1d['ema576'].iloc[-1]
        ...
        levels = [
            ...
            ("S4", price - atr * 3.0, "1H Major Demand / OB Floor"),
        ...
        out_path = os.path.join(self.workspace, f"{asset}_FINAL_V7_5.pdf")
        pdf.output(out_path)
        return out_path

if __name__ == "__main__":
    if len(sys.argv) > 1:
        print(InstitutionalAnalystV7_5().generate_report(sys.argv[1]))
